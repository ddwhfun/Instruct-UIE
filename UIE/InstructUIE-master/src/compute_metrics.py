...
#             if isinstance(pred_data, dict) and 'entities' in pred_data:
#                 # 将JSON格式转换为字符串格式进行比较
#                 pred_str = '; '.join([f"{e['type']}:{e['pos'][0]},{e['pos'][1]}" for e in pred_data['entities']])
#             else:
#                 pred_str = pred
#         except:
#             pred_str = pred
...
#                 gold_str = '; '.join([f"{e['type']}:{e['pos'][0]},{e['pos'][1]}" for e in gold_data['entities']])
#             else:
#                 gold_str = gold
#         except:
#             gold_str = gold
...
#             exact_match_score, prediction=pred_str, ground_truths=gold, xlingual=xlingual
#         )
#         rouge1 += metric_max_over_ground_truths(
#             rouge1_score, prediction=pred_str, ground_truths=gold, xlingual=xlingual
#         )
#         rougeL += metric_max_over_ground_truths(
#             rougeL_score, prediction=pred_str, ground_truths=gold, xlingual=xlingual
#         )
