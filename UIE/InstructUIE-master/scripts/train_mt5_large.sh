...
# 3090 * 4 on t5-700M
CUDA_VISIBLE_DEVICES=1 deepspeed --master_port $port src/run_uie.py \
   --resume_from_checkpoint output/train-mt5-large-clw/checkpoint-200 \
   --do_train \
   --do_predict \
   ...
   --output_dir output/train-mt5-large-clw \
   ...
   --save_strategy steps \
   --save_steps 200
   --ignore_data_skip           # 防止数据重复处理
   --gradient_checkpointing     # 节省20-30%显存
